...
```
26 out of 50 coefficients are changed by more than 10\% when population is excluded. The distribution of estimated coefficients doesn't change a lot with or without population included in the model. I guess we should get a little bit smaller $m$ when we include the population, as suggested by Siyuan's method (see Table 3 and 4 on Overleaf document).
...
Snp_data <- data.frame(CEU=AIM$CEU,YRI=AIM$YRI)
maf <- Snp_data[sample(nrow(Snp_data),K,replace=F),]
sim_data <- simSNPCov_popl(200,Beta,MAF1=maf[,1],MAF2=maf[,2],beta_s=1)
...
ggplot(data, aes(value, fill = var)) + geom_density(alpha = 0.2) + xlim(c(-5,5))
```
Now 43 out of 50 coefficients are changed by more than 10\% when population is excluded. The distribution of estimated coefficients with population included in the model has a much heavier tail than the distribution without population (i.e. coefficients are more extreme when we include population as a confounder). In this case, I guess we should get a much smaller $m$ when we include population. 
