...
MCEM=function(m,data,N) {
  ...
  inital_params=model$coefficients[c(1,3:12)]
  params=matrix(0,ncol=11)
  params=rbind(params,inital_params)
  ...
  Weight=function(beta) {
    ...
    s=params[p,1]+data_m[,3:12]%*%params[p,2:11]+data_m[,2]*beta
    prod(exp(data_m[,1]*s)/(1+exp(s)))
  }
  
  Y=rep(data$case,times=N)
  PC1=rep(data$PC1,times=N)
  PC2=rep(data$PC2,times=N)
  PC3=rep(data$PC3,times=N)
  PC4=rep(data$PC4,times=N)
  PC5=rep(data$PC5,times=N)
  PC6=rep(data$PC6,times=N)
  PC7=rep(data$PC7,times=N)
  PC8=rep(data$PC8,times=N)
  PC9=rep(data$PC9,times=N)
  PC10=rep(data$PC10,times=N)
  betas=log(rf(N,m,m))
  ...
  while(norm(as.matrix(params[p,]-params[p-1,]))>=threshold) {
    ...
    g=glm(Y~offset(O)+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10,
...
lkhdk=function(params_k,data,m,N) {
  ...
  for(j in 1:N) {
    data_m=as.matrix(data)
    s=params_k[1]+data_m[,3:12]%*%params_k[2:11]+data_m[,2]*betas[j]
