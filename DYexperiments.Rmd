...
#Snp_data <- Snp_data[which(abs(Snp_data$CEU-Snp_data$YRI)>=0.2),]
maf <- matrix(nrow=K,ncol=2)
ceu <- sample(Snp_data$CEU,size=K)
...
for (i in 1:K) {
  XX <- sim_data %>% select(c(paste0("X",i),Population,case)) %>% rename(X=paste0("X",i))
  betahat[i] <- coef(glm(case~.,data=XX,family=binomial(),maxit=100))[[2]]
  
  XX0 <- sim_data %>% select(c(paste0("X",i),case)) %>% rename(X=paste0("X",i))
...
X2$var <- "without_popl"
data <- rbind(X1, X2)
ggplot(data, aes(value, fill = var)) + geom_density(alpha = 0.2)
```
Only 20 out of 50 coefficients are changed by more than 10\% when population is excluded. The distribution of estimated coefficients doesn't change a lot with or without population included in the model. I guess this is why our methods are suggesting a similar $m$. 


### What if we only restrict to SNPs whose MAF differs by more than 0.4 between the two populations ? 
```{r}
set.seed(1234)
Snp_data <- Snp_data[which(abs(Snp_data$CEU-Snp_data$YRI) >= 0.4),] 
maf <- matrix(nrow=K,ncol=2);
ceu <- sample(Snp_data$CEU,size=K)
for (i in 1:length(ceu)) {
  maf[i,1] <- ceu[i]
...
ggplot(data, aes(value, fill = var)) + geom_density(alpha = 0.2)
```
Now 47 out of 50 coefficients are changed by more than 10\% when population is excluded. The distribution of estimated coefficients with population included in the model has a heavier tail than the distribution without population (i.e. coefficients are more extreme when we include population as a confounder), and this may result in a smaller estimated $m$ when we include population. 
